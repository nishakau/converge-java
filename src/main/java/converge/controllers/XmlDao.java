...
class XmlDao {
	...
	private static  String READ_XML ="select doc from xml_type where xmlcast(xmlquery('$p/order/id/text()' passing doc AS \"p\" returning content) AS varchar2(5))=?";
	...
	private static String DELETE_XML_BY_ID ="delete from xml_type where xmlcast(xmlquery('$p/order/id/text()' passing doc AS \"p\" returning content) AS varchar2(5))=?";
	
	private static String CHECK_IF_XML_WITH_ID_EXISTS ="select count(*) from xml_type where xmlcast(xmlquery('$p/order/id/text()' passing doc AS \"p\" returning content) AS varchar2(5))=?";
